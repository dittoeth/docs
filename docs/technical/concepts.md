...
This system uses an _orderbook_ to match buy and sell sides (thus two sided) to create a stable asset. A buyer takes their ETH with the intent of getting the stable asset (dUSD, Carbon USD). A [shorter](../overview/glossary#shorter) takes their ETH with the intent of getting the yield of their collateral in addition to the extra collateral of the buyer.
...
| ------------- | ------------ | ------------------- |
| depositdETH   | burns dETH   | +ethEscrowed        |
| depositAsset  | burns ERC-20 | +ercEscrowed        |
| withdrawdETH  | mints dETH   | -ethEscrowed        |
...
**dETH can be redeemed in 3 ways** (fastest to slowest):

- Withdraw to dETH itself
