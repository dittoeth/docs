...
- **onlyDAO**: Verifies the caller is the owner of the diamond contract. Owner will initially be the contract deployers, but will be set to the DAO at a future date.
- **onlyAdminOrDAO**: Verifies the caller is the admin or owner, which allows for quicker responses to time sensitive actions.
...
- **onlyDiamond**: Checks that the caller is the diamond contract. This is to give privileged access to certain external functions.
  ...
  | setOracle         | transferAdminship         | getAssetCollateralRatio | withdraw (bridge)   |
  | setVault          | setTithe                  | getBids                 | unstake (bridge)    |
  | createBridge      | setDittoMatchedRate       | getAsks                 | createForcedBid     |
  | deleteBridge      | setDittoShorterRate       | getShorts               |                     |
  | createMarket      | setInitialCR              | getShortIdAtOracle      |                     |
  | diamondCut ¹      | setPrimaryLiquidationCR   | getShortRecords         |                     |
